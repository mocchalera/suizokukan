...
export function png(width = 32, height = 32, metadata: boolean | 'eXIf' = false): string {
  ...
  return `data:image/png;base64,${Buffer.concat([Buffer.from([137, 80, 78, 71, 13, 10, 26, 10]), chunk('IHDR', header), ...(metadata ? [chunk(metadata === 'eXIf' ? 'eXIf' : 'tEXt', Buffer.from('private\0not allowed'))] : []), chunk('IDAT', deflateSync(pixels)), chunk('IEND', Buffer.alloc(0))]).toString('base64')}`;
}
