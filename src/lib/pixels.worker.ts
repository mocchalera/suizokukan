...
self.onmessage = (event: MessageEvent) => {
  const { id, width, height, buffer, threshold, paperMode, autoLighting } = event.data;
  try {
    const result = removePaper({ width, height, data: new Uint8ClampedArray(buffer) }, threshold, paperMode, autoLighting);
