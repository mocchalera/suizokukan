...
import { createCutoutProcessor } from '../lib/cutout-processor';
import { Sea } from './Sea';
...
export function Editor({ initial, onSave, jev }: { initial?: LocalCreature; onSave: (creature: LocalCreature, notice: string) => Promise<void>; jev: boolean }) {
  ...
  const [threshold, setThreshold] = useState(40); const [paper, setPaper] = useState(false); const [autoLighting, setAutoLighting] = useState(true); const [brush, setBrush] = useState<'keep' | 'erase'>('erase');
  ...
  const processor = useRef<ReturnType<typeof createCutoutProcessor> | null>(null); const alive = useRef(true); const releaseAbort = useRef<AbortController | null>(null);
  ...
  useEffect(() => {
    alive.current = true;
    const instance = createCutoutProcessor(new Worker(new URL('../lib/pixels.worker.ts', import.meta.url), { type: 'module' }), {
      onResult: result => { working.current = result; history.current = []; setHistoryCount(0); refresh(); },
      onError: message => { setError(message); setImage(''); },
      onBusy: setBusy,
    });
    processor.current = instance;
    return () => { alive.current = false; instance.dispose(); processor.current = null; releaseAbort.current?.abort(); original.current = null; working.current = null; history.current = []; };
  }, []);
  const process = (value = threshold, keepPaper = paper, delay = 0, adjustLighting = autoLighting) => {
    if (!original.current || !processor.current) return;
    setError(''); processor.current.request(original.current, value, keepPaper, delay, adjustLighting);
  };
