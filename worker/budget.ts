...
export class DailyBudget extends DurableObject<AppEnv> {
  async fetch(request: Request) {
    ...
    const maximum = kind === 'rooms' ? 40 : Math.max(0, Math.min(50, Number.isFinite(limit) ? Math.floor(limit) : 0));
    if (count(totalKey) >= maximum || count(visitorKey) >= (kind === 'rooms' ? 12 : 10)) return json({ allowed: false });
