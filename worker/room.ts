...
type Row = { id: string; payload: string; digest: string; bytes: number; ready: number; sender: string };
...
export class SeaRoom extends DurableObject<AppEnv> {
  ...
  summary(row: Row) {
    ...
    return { ...creature, ready: !!row.ready, sender: row.sender };
  }
  snapshot() {
  ...
  async fetch(request: Request): Promise<Response> {
    try {
      ...
      if (url.pathname === '/init' && request.method === 'POST') {
        ...
        this.ctx.storage.sql.exec('CREATE TABLE creatures (id TEXT PRIMARY KEY, payload TEXT NOT NULL, digest TEXT NOT NULL, bytes INTEGER NOT NULL, ready INTEGER DEFAULT 0, sender TEXT)');
      ...
      if (route.action === 'creatures' && route.creatureId && route.image && request.method === 'GET') {
        ...
        return new Response(Uint8Array.from(atob(image.slice(22)), value => value.charCodeAt(0)), { headers: { 'Content-Type': 'image/png', 'Cache-Control': 'no-store', 'X-Content-Type-Options': 'nosniff', 'Cross-Origin-Resource-Policy': 'same-origin' } });
      }
      ...
      if (route.action === 'creatures' && !route.creatureId && request.method === 'POST') {
        ...
        if (duplicate === 'same') return json({ id: creature.id, ready: !!existing.ready, duplicate: true });
        const rows = this.rows(); const bytes = atob(creature.image.slice(22)).length;
        if (!canAdd(rows.length, rows.reduce((sum, row) => sum + row.bytes, 0), bytes)) throw new HttpError(409, 'このうみは20匹でいっぱいです。');
        this.ctx.storage.sql.exec('INSERT INTO creatures (id,payload,digest,bytes,ready,sender) VALUES (?,?,?,?,0,?)', creature.id, payload, payloadDigest, bytes, sender);
  ...
  async webSocketMessage(socket: WebSocket, message: string | ArrayBuffer) {
    try {
      ...
      if (input.type === 'ready' && info.role === 'owner' && isId(input.id)) {
        this.ctx.storage.sql.exec('UPDATE creatures SET ready = 1 WHERE id = ?', input.id);
        this.broadcast({ type: 'ready', id: input.id }); return;
      }
